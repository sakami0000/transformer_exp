...
class DecoderEmbeddings(nn.Module):
    ...
    def forward(
        ...
        numerical_embeddings = self.numerical_linear(numerical_states)
...
class SaintEncoderDecoderModel(nn.Module):
    ...
    def tfixup(self):
        ...
        for name, param in self.named_parameters():
            ...
            elif name.endswith("_embeddings.weight"):
                param.data.normal_(
                    mean=0.0,
                    std=(
                        (0.67 * self.encoder_config.num_hidden_layers) ** (-1.0 / 4.0)
        ...
        for name, param in self.named_parameters():
            if name.endswith(".dense.weight"):
                nn.init.xavier_normal_(
                    param,
                    gain=(9 * self.decoder_config.num_hidden_layers) ** (-1.0 / 4.0),
                )
            elif name.endswith(".value.weight"):
                nn.init.xavier_normal_(
                    param,
                    gain=(
                        (9 * self.decoder_config.num_hidden_layers) ** (-1.0 / 4.0)
                        * (2 ** 0.5)
                    ),
                )
            elif name.endswith("_embeddings.weight"):
                param.data.normal_(
                    ...
                    std=(
                        (9 * self.decoder_config.num_hidden_layers) ** (-1.0 / 4.0)
                        * self.decoder_config.embedding_size ** (-0.5)
                    ),
                )
        ...
        #     modules[k] = v
        #     parts = k.split(".")
        #     father = modules[".".join(parts[:-1])]
        #     child_name = parts[-1]
        #     if isinstance(v, nn.LayerNorm):
        #         setattr(father, child_name, nn.Identity())
