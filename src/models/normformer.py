...
class SaintAttention(BertAttention):
    ...
    def scale_heads(self, context_layer: torch.FloatTensor) -> torch.FloatTensor:
        concat_size = context_layer.size()
        context_layer_size = context_layer.size()[:2] + (
            self.self.num_attention_heads,
            self.self.attention_head_size,
        )

        context_layer = context_layer.view(*context_layer_size)
        context_layer = torch.einsum("bthd,h->bthd", context_layer, self.head_scale)
        context_layer = context_layer.view(*concat_size)
