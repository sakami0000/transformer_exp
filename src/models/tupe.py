...
class DecoderEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_embed = nn.Linear(
            config.response_embedding_size + config.embedding_size * 3,
            config.hidden_size,
        )
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class SaintModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.position_scaling = (
            float(config.hidden_size / config.num_attention_heads) ** -0.5
        )
        self.position_layer_norm = nn.LayerNorm(
    ...
    def compute_bias(self, position_ids):
        ...
        position_query = self.position_query_linear(
            position_embeddings
        )  # (batch_size, seq_length, hidden_size)
        position_key = self.position_key_linear(
            position_embeddings
        )  # (batch_size, seq_length, hidden_size)

        position_query = (
            self.transpose_for_scores(position_query) * self.position_scaling
        )  # (batch_size, num_attention_heads, seq_length, attention_head_size)
        position_key = self.transpose_for_scores(
            position_key
        )  # (batch_size, num_attention_heads, seq_length, attention_head_size)

        absolute_bias = torch.matmul(
            position_query, position_key.transpose(-1, -2)
        ...
        relative_position_bucket = self._relative_position_bucket(
            relative_position,  # (batch_size, query_length, key_length)
            bidirectional=False,
        ...
        relative_bias = self.relative_attention_bias(
            relative_position_bucket
        )  # (batch_size, query_length, key_length, num_heads)
