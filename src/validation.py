...
def virtual_time_split(
    train_df: pd.DataFrame, valid_size: int, epoch_valid_size: int
    ...
    virtual_timestamp_user = np.random.rand(len(max_timestamp_user)) * (
        max_timestamp_user.max() - max_timestamp_user
    )
    virtual_timestamp = (
        virtual_timestamp_user.reindex(train_df["user_id"]).reset_index(drop=True)
        + train_df["timestamp"]
    )
