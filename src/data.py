...
def get_user_sequences(df: pd.DataFrame) -> Dict[int, Dict[str, List[int]]]:
    ...
    df_sorted["elapsed_time"] = (
        df["prior_question_elapsed_time"].fillna(0) / 1000
    )  # seconds
...
class ValidDataset(Dataset):
    ...
    def _get_valid_idx_map(
        train_df: pd.DataFrame, valid_idx: np.ndarray
    ...
    def get_last_slice(
        ...
        if user_id in self.train_user_seqs and self.window_size > list_idx:
            ...
            train_content_id = self.train_user_seqs[user_id]["content_id"][
                -train_sequence_length:
            ]
            train_part = self.train_user_seqs[user_id]["part"][-train_sequence_length:]
            train_timestamp = self.train_user_seqs[user_id]["timestamp"][
                -train_sequence_length:
            ]
            train_elapsed_time = self.train_user_seqs[user_id]["elapsed_time"][
                -train_sequence_length:
            ]
            train_target = self.train_user_seqs[user_id]["answered_correctly"][
                -train_sequence_length:
            ]

        start_idx = max(list_idx - self.window_size, 0)
        valid_content_id = self.valid_user_seqs[user_id]["content_id"][
            start_idx:list_idx
        ]
        ...
        valid_elapsed_time = self.valid_user_seqs[user_id]["elapsed_time"][
            start_idx:list_idx
        ]
        valid_target = self.valid_user_seqs[user_id]["answered_correctly"][
            start_idx:list_idx
        ]
        ...
        content_id = (
            train_content_id
            + valid_content_id[:-same_timestamp_count]
            + [valid_content_id[-1]]
        )
        part = train_part + valid_part[:-same_timestamp_count] + [valid_part[-1]]
        timestamp = (
            train_timestamp
            + valid_timestamp[:-same_timestamp_count]
            + [valid_timestamp[-1]]
        )
        elapsed_time = train_elapsed_time + valid_elapsed_time
        target = [1] + [
            label + 2 for label in train_target + valid_target[:-same_timestamp_count]
        ]  # add start id
